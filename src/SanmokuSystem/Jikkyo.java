...
public class Jikkyo extends JPanel implements MouseListener {
	...
	Jikkyo(Server server, int nplayer, int board_size, String[] player_names) {
		this.server = server;
	...
	void paint_wild(Graphics2D g2, int v, int x, int y) {
		g2.setColor(new Color(255, 192, 0));
		int[] xx = new int[10];
		int[] yy = new int[10];
		double r = radius / 4;
		for (int i = 0; i < 10; i++) {
			xx[i] = x + (i % 2 + 1) * (int) (r * Math.cos(Math.toRadians(36 * i + 90)));
			yy[i] = y + (i % 2 + 1) * (int) (r * Math.sin(Math.toRadians(36 * i + 90)));
		}
		g2.fillPolygon(xx, yy, 10);
	}
	...
	public void paint(Graphics g) {
		...
		for (int y = 0; y < board_size; y++) {
			for (int x = 0; x < board_size; x++) {
				...
				int v = board[i];
				if (v == Server.WILD_STONE) {
					paint_wild(g2, v, x * delta + margin + (delta / 2), y * delta + margin + (delta / 2));
				} else if (v != -1) {
		...
		for (int i = 0; i < nplayer; i++) {
			...
			// y += (dy/2);
			// String scores = current_score[i] + " / " + game_score[i] + " " +
			// player_names[i];
			if (server.player_left[i] < 0) {
			...
			// String scores = String.format("%d / %2d %s", current_score[i], game_score[i],
			// player_names[i]);
			...
			String attackable = (server.isAttackable(i)) ? "+" : "-";
			String left = String.format("%2d",server.player_left[i]);
			String information = String.format("%s %s  : %s", left,attackable,scores);
			g2.drawString(player_names[i], x + radius + margin / 4, y + (radius / 2));
			g2.drawString(information, x + radius + margin, y + (radius / 2) + 16);
		}
	}

	/*
	 * int x = current_pos % board_size; int y = current_pos / board_size; try{
	 * for(int i = 0; i < 3; i++){ Thread.sleep(500);
	 * g2.setColor(g2.getBackground()); g2.fillOval(x * delta + margin + (delta / 2)
	 * - (radius / 2), y * delta + margin + (delta / 2) - (radius / 2), radius,
	 * radius); Thread.sleep(500); g2.setColor(Color.black); g2.fillOval(x * delta +
	 * margin + (delta / 2) - (radius / 2), y * delta + margin + (delta / 2) -
	 * (radius / 2), radius, radius); } } catch(InterruptedException e){ }
	 */

	void startgame(int game_no) {
		JOptionPane.showMessageDialog(this, game_no + 1 + "回戦\ngame start");
	}

	void endgame() {
		JOptionPane.showMessageDialog(this, "game end" + makeWinnersList(server.get_current_score()));
	}

	void endmatch() {
		draw_panel();
		JOptionPane.showMessageDialog(this, "match end" + makeWinnersList(server.get_game_score()));
		frame.dispose();
	}
