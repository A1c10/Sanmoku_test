...
public class Server {
	...
	private boolean isPlayersStonesLeft() {
		boolean result = false;
		for (int s : player_left) {
			result |= (s > 0);
		}
		return result;
	}

	public static final int ATTACK = 1000; // 指し手の座標指定の際に付加する攻撃用フラグ値
	private boolean[] player_attackable;

	// プレーヤ向けの public methods

	public boolean isAttackable(int id) {
		return player_attackable[id];
	}
	...
	public static final int WILD_STONE = 1000;
...
//			}
		...
		for (int i = nconsumed; i < args.length; i++) {
			...
			try {
				...
				if (name.charAt(0) == '*') {
					...
					p = (Player) Class.forName("Player." + name).getDeclaredConstructor().newInstance();
				}
				players[id] = p;
	...
	private boolean play_ng(int id, int pos) {
		if (pos < ATTACK) { // 攻撃モードでなければ従来通り
			...
			return !isAttackable(id); // 攻撃可でなければNG(true)
		}
	}

	/**
	 * プレーヤ p の turn メソッドを呼び出して着手をもらう もらった着手が盤外だったら AssertionError を投げる
	 * 
	 * @param p     プレーヤオブジェクト
	...
	/**
	 * プレーヤの turn メソッドを呼び出し、結果を返す 指定した時間を超過したら強制終了して -1 を返す 例外を発生したりアサーションに失敗しても -1
	 * を返す 合法な着手でない場合、繰り返して turn を呼び出す
	 * 
	 * @param nturn  手番 (初手: 0)
			...
			public void run() {
				try {
					...
					while ((nturn > 0) && play_ng(id, tmp_result)) {
						...
						if (this.toBeKilled) {
							if (enable_jikkyo) {
								String msg = String.format("ERROR (same position(%d) answer repeat): %s%n", tmp_result,
										p);
								errConsole.addText(msg);
								p.consoleOutln(msg);
							}
					...
					ex.printStackTrace();
					if (enable_jikkyo) {
						String msg = String.format("ERROR (internal failure): %s%n", p);
						errConsole.addText(msg);
						p.consoleOutln(msg);
					}
					result = -1;
	...
	private void game(int game_no) {
		...
		Arrays.fill(board, -1);
		board[new Random().nextInt(board_size * board_size)] = WILD_STONE;
		board[new Random().nextInt(board_size * board_size)] = WILD_STONE;
		current_score = calc_score();

		player_left = new int[nplayer];
		player_attackable = new boolean[nplayer];

		int n = 0;
		int start_player = game_no;

		for (int i = 0; i < nplayer; i++) {
			player_left[i] = nstone;
			player_attackable[i] = true;
		...
		jikkyo.startgame(game_no);

		while (isPlayersStonesLeft()) {
//		for (int s = 0; s < nstone; s++) {
			int[] shuffleTable = new int[nplayer];
			boolean[] useFlags = new boolean[nplayer];

			if (n == 0) {
				// int id = (p + start_player) % nplayer;
				for (int i = 0; i < nplayer; i++) {
					shuffleTable[i] = (i + start_player) % nplayer;
				}
			} else {
				for (int i = 0; i < nplayer; i++) {
					useFlags[i] = false;
				...
				for (int i = 0; i < nplayer; i++) {
					int index = rnd.nextInt(nplayer);
					while (useFlags[index]) {
						System.out.printf("%d ", index);
						index = rnd.nextInt(nplayer);
					}
					System.out.printf("%d/%d\n", index, i);
					shuffleTable[i] = index;
					useFlags[index] = true;
				}
			}
			...
			for (int p = 0; p < nplayer; p++) {
				...
				if (player_left[id] > 0) {
					...
					if (jikkyo != null) {
						jikkyo.turn_start(id, player_left[id]);
					}
					...
					if (pos < 0) {
						...
						if (pos < ATTACK) {
							...
							player_attackable[id] = false;
							pos %= ATTACK;
							int preStone = board[pos];
							board[pos] = -1;
							// todo 取られたプレーヤーの救済
							if ((0 <= preStone) && (preStone < nplayer)) {
								player_left[preStone]++;
							}
						}
						current_score = calc_score();
						if (jikkyo != null) {
